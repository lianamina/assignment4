 ...
 */
   ...
   */
    ...
    if (sock->type == TCP_INITIATOR) {
        ...
        if (flags & SYN_FLAG_MASK) {
            ...
            // Send window setup
            sock->send_win.last_ack = get_seq(hdr);
   ...
   else if (get_ack(hdr) == sock->send_win.last_ack)
   {
     ...
     else // `Slow start` or `Congestion avoidance` state
     {
       ...
       */
        ...
        {
          sock->cong_win += MSS; // fast recovery
        }
        else
        {
          sock->dup_ack_count++;
  ...
  fprintf(stderr, "[HANDLE_PKT] flags: %02x, ack: %u, seq: %u, plen: %u\n",
   ...
   if (!sock->complete_init)
   {
    ...
    if (flags & ACK_FLAG_MASK) {
        ...
        if (sock->send_fin_seq > 0 && get_ack(hdr) == sock->send_fin_seq + 1) {
 ...
 void send_pkts_data(ut_socket_t *sock)
 {
   ...
   */
    ...
    uint32_t window = MIN(sock->cong_win, adv_win);
    ...
    while (in_flight < window && in_flight < sock->sending_len)
    {
